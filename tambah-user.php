...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $insert = mysqli_query($koneksi, "INSERT INTO user (nama, email, password, id_level, username, foto) VALUES ('$nama','$email','$password', '$id_level', '$username', '$nama_foto')");
        }
    } else {
                                                    ...
                                                    <select name="id_level" id="" class="form-control">
                                                        ...
                                                        <?php while ($rowLevels = mysqli_fetch_assoc($queryLevels)): ?>
                                                            <option <?php echo isset($_GET['edit']) ? ($rowLevels['id'] == $rowEdit['id_level'] ? 'selected' : '') : '' ?> value="<?php echo $rowLevels['id'] ?>">
                                                                <?php echo $rowLevels['nama_level'] ?></option>
                                                        <?php endwhile ?>
                                                    </select>
                                                </div>
                                                <div class="col-sm-6 mt-2">
                                                    <label for="" class="form-label">Username</label>
                                                    <input type="text" class="form-control" name="username" placeholder="Masukkan Username Anda" required value="<?php echo isset($_GET['edit']) ? $rowEdit['username'] : '' ?>">
                                                </div>
                                            </div>
                                            <div class="mb-3 row">
                                                <div class="col-sm-12">
